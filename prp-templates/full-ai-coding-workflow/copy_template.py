...
def get_template_files() -> List[Tuple[str, str]]:
    ...
    # Claude commands directory - copy ALL commands
    ...
    # Claude agents directory - copy ALL agents
    agents_dir = template_root / ".claude" / "agents"
    if agents_dir.exists():
        for file in agents_dir.glob("*.md"):
            rel_path = f".claude/agents/{file.name}"
            files_to_copy.append((str(file), rel_path))
    
    # Claude settings file if exists
    settings_file = template_root / ".claude" / "settings.local.json"
    if settings_file.exists():
        files_to_copy.append((str(settings_file), ".claude/settings.local.json"))
    ...
    if prps_dir.exists():
        ...
        # Copy INITIAL-EXAMPLE.md
        initial_example_file = prps_dir / "INITIAL-EXAMPLE.md"
        if initial_example_file.exists():
            files_to_copy.append((str(initial_example_file), "PRPs/INITIAL-EXAMPLE.md"))
        
        # Copy ALL examples from PRPs/examples directory (excluding venv and cache)
        prp_examples_dir = prps_dir / "examples"
        if prp_examples_dir.exists():
            for example_dir in prp_examples_dir.iterdir():
                if example_dir.is_dir():
                    # Copy all files in each example directory recursively
                    for file in example_dir.rglob("*"):
                        if file.is_file():
                            # Skip files in venv directories
                            if "venv" in file.parts or "venv_linux" in file.parts or ".venv" in file.parts:
                                continue
    ...
    # PromptProgression directory - copy all files
    prompt_dir = template_root / "PromptProgression"
    if prompt_dir.exists():
        for file in prompt_dir.glob("*.md"):
            rel_path = f"PromptProgression/{file.name}"
            files_to_copy.append((str(file), rel_path))
    
    # FullExample directory - copy entire structure (excluding venv)
    full_example_dir = template_root / "FullExample"
    if full_example_dir.exists():
        for file in full_example_dir.rglob("*"):
            if file.is_file():
                # Skip files in venv directories
                if "venv" in file.parts or "venv_linux" in file.parts or ".venv" in file.parts:
                    continue
                ...
                if "__pycache__" in file.parts or file.suffix == ".pyc":
                    continue
                # Skip .env files (should use .env.example)
                if file.name == ".env":
                    continue
                rel_path = file.relative_to(template_root)
                files_to_copy.append((str(file), str(rel_path)))
    
    # .devcontainer directory - copy all files
    devcontainer_dir = template_root / ".devcontainer"
    if devcontainer_dir.exists():
        for file in devcontainer_dir.rglob("*"):
            if file.is_file():
                rel_path = file.relative_to(template_root)
                files_to_copy.append((str(file), str(rel_path)))
...
def validate_template_integrity(target_dir: Path) -> bool:
    ...
    essential_files = [
        ...
        # Agents
        ".claude/agents/pydantic-ai-dependency-manager.md",
        ".claude/agents/pydantic-ai-prompt-engineer.md",
        ".claude/agents/pydantic-ai-tool-integrator.md",
        ".claude/agents/pydantic-ai-validator.md",
        # PRPs
        "PRPs/templates/prp_pydantic_ai_base.md",
        "PRPs/INITIAL.md",
        "PRPs/INITIAL-EXAMPLE.md",
        # Examples
        "PRPs/examples/basic_chat_agent/agent.py",
        "PRPs/examples/main_agent_reference/settings.py",
        "PRPs/examples/rag_pipeline/sql/schema.sql",
        "PRPs/examples/testing_examples/test_agent_patterns.py",
        # PromptProgression
        "PromptProgression/bad_prompt.md",
        "PromptProgression/task_prompt_8_prp.md",
        # FullExample
        "FullExample/README.md",
        "FullExample/agent/agent.py"
    ]
